--[[
    XYLENT HUB v1.0 (COMPLETE - OPTIMIZED)
    Dev: MASGAL x DRKY  |  Team: XYLENT TEAM
    Silent Aim sudah dioptimasi - No Lag & No Error Spam
]]
...
local function mkDropdown(page, label, options, order, cb)
    ...
    for idx, opt in ipairs(options) do
        ...
        ob.Font             = Enum.Font.GothamBold        ob.TextSize         = 9
...
local deleteToggle, setDelete = mkToggle(pMain, "Delete Wall Mode", "Tekan E untuk hapus, U untuk undo", 11, function(v)
    deleteWallEnabled = v
end)
...
RunService.RenderStepped:Connect(function()
    if deleteWallEnabled then
        ...
        if target and target:IsA("BasePart") then
            local isImportant = target:IsDescendantOf(lp.Character) or
...
-- ================================================================
-- PAGE 2 — COMBAT (SILENT AIM OPTIMIZED - NO LAG)
-- ================================================================
...
local function GetFovTargetFast()
    local now = tick()
    if lastTarget and (now - lastTargetTime) < 0.15 then
                    ...
                    if target and target.Character then
                        ...
                        if targetPart then
                            ...
                            if SilentAimWallbang then
                                if newDirection.Magnitude <= MaxWallbangDistance then
...
local saToggle, setSAToggle = mkToggle(pCombat, "Silent Aim", "Auto-aim ke target dalam FOV", 2, function(v)
    SilentAim = v
    if not v then lastTarget = nil end
end)
...
local wallbangToggle, setWallbang = mkToggle(pCombat, "Wallbang", "Shoot through walls", 6, function(v)
    SilentAimWallbang = v
end)
mkSlider(pCombat, "Wallbang Distance", 10, 5000, 500, 7, function(v) MaxWallbangDistance = v end)
...
local aimbotToggle, setAimbot = mkToggle(pCombat, "Aimbot", "Magnet mouse ke target", 11, function(v)
    aimbotEnabled = v
...
mkSlider(pCombat, "Aimbot FOV", 10, 400, 200, 12, function(v) aimbotFov = v end)
...
for i, loc in ipairs(vtpLocs) do
    mkBtn(pVTeleport, loc[1], i+1, nil, function() tpVehicle(loc[2], loc[3], loc[4]) end)
end

local statusCard = mkCard(pVTeleport, 36, #vtpLocs + 2)
...
task.spawn(function()
    local t = 0    while thL and thL.Parent do
        t = t + 0.025
...
print("✅ XYLENT HUB v1.0 OPTIMIZED - No Lag, No Error Spam!")
